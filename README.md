...
Agent 已经成为下一代软件开发的基础设施。claw-code 用 Rust 重写了 Claude Code 的核心运行时，约 11.6 万行代码分布在 11 个 crate 中。本书的目标是让有一定系统开发经验的工程师能读懂这套代码，深入到每个模块的设计决策和源码实现。
...
| 第 2 章 | 整体架构全景 | 11 个 crate 的模块地图、数据流、依赖关系、目录结构速查 |
...
| 第 6 章 | Turn Loop 与对话引擎 | ConversationRuntime 泛型设计、run_turn 循环、系统提示词构建 |
| 第 7 章 | 工具系统 | ToolSpec 规范、GlobalToolRegistry 三层注册、55 个工具清单、执行分发 |
| 第 8 章 | MCP 协议与外部工具 | McpServerManager 生命周期、六种传输方式、插件系统、降级启动 |
| 第 9 章 | 权限系统 | PermissionMode 五级模型、PermissionPolicy 规则引擎、TrustResolver |
| 第 10 章 | 会话管理 | Session 状态机、JSONL 持久化、自动压缩、会话分叉 |
| 第 11 章 | Hooks系统 | HookRunner 三事件生命周期、权限覆盖、JSON 协议、取消信号 |
| 第 12 章 | 任务与团队注册表 | TaskRegistry、LaneBoard、Team/Cron 注册表、PolicyEngine 规则决策 |
