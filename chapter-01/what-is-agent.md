...
claw-code 的权限系统（第8章）是 LLM 行为的约束层，它在 Agent 尝试调用工具前拦截请求，根据配置的规则判断这个操作是否被允许。`ReadOnly` 模式禁止任何写操作，`WorkspaceWrite` 只允许修改工作区目录，`DangerFullAccess` 才放开所有限制。这种分级设计承认了一个事实：LLM 的自主性是有风险的，必须通过外部约束来管理。
