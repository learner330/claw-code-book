...
这个循环和常见的请求-响应模式有本质区别。传统的 HTTP 服务处理完一个请求就结束了，下一个请求是新的一次处理。Agent 的 Turn Loop 在 LLM 调用工具后不会结束，而是把工具结果喂回 LLM，让 LLM 决定是否继续调用更多工具。Turn Loop 的完整源码实现分析在第11章。

循环的终止以"LLM 判断任务完成"为标志，而不是"代码执行完毕"。这个判断由 LLM 自主做出，人类不干预每一步的决策。但人类可以通过权限系统（第8章）和Hooks系统（第10章）在关键节点插入干预——权限系统在工具执行前做安全审查，Hooks系统在工具执行前后插入自定义处理逻辑。
