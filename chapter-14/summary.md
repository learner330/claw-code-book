...
`McpServerManager` 的 `discover_tools_best_effort`（第7章）实现降级启动——部分服务器失败时 Agent 仍然可用。`spawn_tool_call` 在新线程创建 tokio 运行时执行异步 MCP 调用。`McpToolRegistry` 通过 `OnceLock` 懒初始化管理器。
